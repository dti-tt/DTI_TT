...
class MoleculeDataset(Dataset):
    ...
    def __init__(self, root_path, dataset, dataset_type, task_names, path_length=5, n_virtual_nodes=2, split_name=None, split=None):
        # 🐵 jungmin: changed path
        dataset_path = os.path.join(root_path, f"{dataset}.csv")
        self.cache_path = os.path.join(root_path, f"{dataset}_{path_length}.pkl")
        split_path = os.path.join(root_path, f"splits/{split_name}.npy")
        ecfp_path = os.path.join(root_path, f"rdkfp1-7_512.npz")
        md_path = os.path.join(root_path, f"molecular_descriptors.npz")
        ...
        self.task_names = task_names #['pIC50']
