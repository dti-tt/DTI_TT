...
def encode_drug(df_data, drug_encoding, column_name = 'SMILES', save_column_name = 'drug_encoding'):
	...
	if drug_encoding == 'Morgan':
		...
		unmatched_values = unmatched_rows[column_name].unique()
		...
		if unmatched_values.size > 0:
			...
			kpgt = KPGTProcessor(unmatched_csv_path, ['Label'])
			unmatched_features = kpgt.get_features()
			unmatched_dict = dict(zip(unmatched_values, unmatched_features))
...
def generate_config(drug_encoding = None, target_encoding = None, 
	...
	if drug_encoding == 'Morgan':
		...
		base_config['input_dim_drug'] = 2304
		base_config['mlp_hidden_dims_drug'] = mlp_hidden_dims_drug
	...
	if target_encoding == 'AAC':
		...
		base_config['input_dim_protein'] = 1024
		base_config['mlp_hidden_dims_target'] = mlp_hidden_dims_drug
